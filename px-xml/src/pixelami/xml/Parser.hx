...
class Parser
{
	static public function parse(str:String, ?file:String)
	{
		var doc = Xml.createDocument();
		trace(file);
		doParse(str, 0, doc, file);
		return doc;
	}
	
	static function doParse(str:String, p:Int = 0, ?parent:Xml, ?file:String):Int
	{
		var xml:Xml = null;
		var state = S.BEGIN;
		var next = S.BEGIN;
			...
			switch(state)
			{
				...
				case S.TAG_NAME:
					if (!isValidChar(c))
					{
						...
						xml.set("__start", Std.string(nodeStart));
						xml.set("__file", file);
						parent.addChild(xml);
						state = S.IGNORE_SPACES;
						next = S.BODY;
