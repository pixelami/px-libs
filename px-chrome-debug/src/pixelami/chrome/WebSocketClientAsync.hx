...
class WebSocketClientAsync implements IWebSocketClient
{
    ...
    var sendQueue:Deque<Dynamic>;
    var recieveQueue:Deque<Dynamic>;
    ...
    public function new()
    {
        ...
        sendQueue = new Deque<Dynamic>();
        recieveQueue = new Deque<Dynamic>();
    }
    ...
    public function connect(socketUrl:String)
    {
        ...
        if(response.status == "101")
        {
            ...
            var t:neko.vm.Thread = neko.vm.Thread.create(readSocketAsync);
            t.sendMessage(neko.vm.Thread.current());
        }
        else
        {
    ...
    public function send(data:Dynamic):Dynamic
    {
        ...
        {
            payloadBytes.writeString(Std.string(data));
            f.payload = payloadBytes.getBytes();
